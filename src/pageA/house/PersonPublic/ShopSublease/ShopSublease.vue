		...
		<view class="content" v-if="showview">
			<view style="padding: 10rpx;">
				<u-form :model="model" :rules="rules" ref="uForm" :errorType="errorType">
					...
					<u-form-item :leftIconStyle="{color: '#888', fontSize: '16rpx'}" label-width="130" :label-position="labelPosition"
					 label="水费" prop="waterRates">
						<view style="display: flex;">
							<view style="width: 98%;">
								<u-input :border="border" placeholder="请输入水费" v-model="model.waterRates" type="text"></u-input>
							</view>
							<text>元</text>
						</view>
					</u-form-item>
					<u-form-item :leftIconStyle="{color: '#888', fontSize: '16rpx'}" label-width="130" :label-position="labelPosition"
					 label="转让费" prop="assignmentFee">
						<view style="display: flex;">
							<view style="width: 98%;">
								<u-input :border="border" placeholder="请输转让费" v-model="model.assignmentFee" type="text"></u-input>
							</view>
							<text>元</text>
						</view>
					</u-form-item>
					...
					<u-form-item :leftIconStyle="{color: '#888', fontSize: '16rpx'}" label-width="130" :label-position="labelPosition"
					 label="剩余租期" prop="remainingLease ">
						<u-input :border="border" type="select" :select-open="selectShow7" v-model="model.remainingLease" placeholder="请选择剩余租期"
					...
					<u-form-item :leftIconStyle="{color: '#a0cfff', fontSize: '16rpx'}" label-width="130" :label-position="labelPosition"
					 label="客流人群" prop="trafficCrowds">
						<u-checkbox-group @change="checkboxGroupChange1">
							<u-checkbox @change="checkboxChange1" v-model="item.checked" v-for="(item, index) in list2" :key="index" :name="item.name">{{item.name}}</u-checkbox>
						</u-checkbox-group>
					</u-form-item>
...
<script>
	...
	import { addShopSubletByPerson } from '@/util/house/shop.js'
	import {
		findAllEstateByCity
	...
	export default {
		...
		methods: {
									...
									success: (res) => {
										...
										if (i === this.model.photo.length - 1) {
											addShopSubletByPerson({
												...
												"rentFreeMonth": this.model.rentFreeMonth,//转让费
												...
												if (res.data.code === 0) {
													setTimeout(() => {
														uni.hideLoading()
														uni.reLaunch({
															url: '/pageA/house/mypage/myShopList/myShopList'
														})
													}, 1500);
												} else {
			...
			addEstate() {
				uni.navigateTo({
					url: '/house/mypage/agentPublic/addEstate/addEstate'
				})
			},
			// 上传图片
			uploadImage() {
			...
			// 选择商铺剩余租期回调
			selectConfirm7(e) {
				this.model.remainingLeaseId = e[0].value;
				this.model.remainingLease = '';
				e.map((val, index) => {
					this.model.remainingLease += this.model.remainingLease == '' ? val.label : '/' + val.label;
					// this.model.houseTypeId = e[index].value
				})
			},
			// 选择商铺压几付几回调
