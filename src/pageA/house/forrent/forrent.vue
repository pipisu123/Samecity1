<template>
	<view class="">
		<view class="content" v-if="showview">
			...
			<u-form :model="model" :rules="rules" ref="uForm" :errorType="errorType">
				...
				<u-form-item :leftIconStyle="{color: '#a0cfff', fontSize: '32rpx'}" left-icon="home" label-width="120"
				 :label-position="labelPosition" label="小区" prop="area">
					<u-input :border="border" placeholder="请输入小区" v-model="model.area" type="select" @click="showCommunity"></u-input>
				...
				<u-form-item :label-position="labelPosition" label="押金方式" prop="coun" label-width="150" color="#a0cfff">
					<u-input :border="border" type="select" :select-open="selectShow7" v-model="model.coun" placeholder="请选择押金方式"
				...
				<u-form-item :label-position="labelPosition" label="租房类型" prop="zuf" label-width="150" color="#a0cfff">
					<u-input :border="border" type="select" :select-open="selectShow5" v-model="model.zuf" placeholder="请选择租房类型"
				...
				<u-form-item :label-position="labelPosition" label="看房时间" prop="Type" label-width="150" color="#a0cfff">
					<u-input :border="border" type="select" :select-open="selectShow6" v-model="model.Type" placeholder="请选择看房时间"
				...
				<text style="color: #a0cfff;">可上传多张图片，大小不超100m</text>
			...
			<view class="public">
				<u-button type="primary" @click="submit" throttle-time=3000>立即发布</u-button>
			</view>
		...
		<view class="" v-else>
			...
			<view class="bottom-nav">
				<u-button type="primary" @click="ToaddCommunity">添加小区(如未搜索出添加即可)</u-button>
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			ToaddCommunity() {
				uni.navigateTo({
					url: '/pageA/house/mypage/addCommunity/addCommunity'
				})
			},
			// 选择回调出小区id
			getid(e) {
			...
			getVal(val) {
				this.showview = true
				this.model.area = val.lable
				this.model.communityId = val.value
			},
			getInputVal() {
			...
			toCreate() {
				uni.navigateTo({
					url: '/house/brokerpage/brokerpage'
				})
			},
			confirm() {
				uni.navigateTo({
						...
						uni.uploadFile({
							...
							success: (res) => {
								...
								addLease({
									...
									"appliance": this.model.appliances, //家具
									...
									if (res.data.code === 0) {
										setTimeout(() => {
											uni.hideLoading()
											uni.reLaunch({
												url: '/pageA/house/myrelease/myrelease'
											})
										}, 1500);
									} else {
