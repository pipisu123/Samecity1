<template>
	<view class="wrap">
		<u-modal ref="uModal" v-model="show" :show-cancel-button="true" :async-close="asyncClose" title="提示" @confirm="confirm(data.phone)"
		 :content="'是否拨打'+data.phone" confirm-text="确定">
		</u-modal>
		...
		<view class="map">
			...
			<view class="house-map">
				<map class="map1" :longitude="longitude" :latitude="Latitude" :markers="markers" @tap="Navigation(data.community)"></map>
			</view>
		</view>
		<!-- 房屋设施 -->
		...
		<Bar @unCollect="unCollect(data.id)" :isFavorites="isFavorites" @Collect="Collect(data.id)" @gochat="gochat(data.userId)"
		 @toCall="toCall(data.phone)" @share="share"></Bar>
		<u-toast ref="uToast" />
	</view>
</template>
...
<script>
	...
	import Bar from '../common/bottomBar.vue'
	import {
		getLeaseDetails
