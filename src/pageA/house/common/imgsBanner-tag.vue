<template name='imgsBanner'>
		<view class="imgsBannerBox" v-if="video != ''">
			<swiper class="imgsBanner_swiper" :current='comCurrentImg' @change='changCurrent'>
				<swiper-item>
					<video @fullscreenchange="event" loop :muted="muted"  autoplay style="width: 100%;" :src="video" controls></video>
				</swiper-item>
			...
			<!-- 图片位置 -->
			<view class="imgLength">{{(comCurrentImg+1)+'/'+(imgList.length+1)}}</view>
			<scroll-view scroll-x="true" class="scrollImgBox" :scroll-left='scrollImgList' scroll-with-animation>
				<view class="scrollImgList">
					<image :src="item" mode="aspectFill" v-for="(item,index) in imgList" :key='index' :class="comCurrentImg==index+1?'activeImageItem':''" @click="onClickImg(index+1)" :id="'item'+index+1"></image>
				</view>
			</scroll-view>
		</view>
		<!--  -->
		<view class="imgsBannerBox" v-else>
			<swiper class="imgsBanner_swiper" :current='comCurrentImg' @change='changCurrent'>
				<swiper-item v-for="(item,index) in imgList" :key='index'>
					<image :src="item" mode="aspectFill"></image>
				</swiper-item>
